...
public class TheRoomOfRelics {
    private static TheRoomOfRelics instance;
    RelicOfLife relicOfLife;
    RelicOfProtection relicOfProtection;
    RelicOfFury relicOfFury;
    DBConnection db = DBConnection.getInstance();

    private TheRoomOfRelics() {
    }

    public static TheRoomOfRelics getInstance() {
        if (instance == null) {
            instance = new TheRoomOfRelics();
        }
        return instance;
    }

    public void enterRoomOfRelics(AAttributes attacker, Scanner sc) {
        roomOfRelicsOption(attacker, sc);
    }

    private void roomOfRelicsOption(AAttributes attacker, Scanner sc) {
    ...
    private void digForRelic(AAttributes attacker, Scanner sc) {
        ...
        do {
            println(PURPLE + "\nPress \"Enter\" to dig! ╰┈➤1.Return to Rooms" + RESET);
            sc.nextLine();
            countTimeForDig();
    ...
    private boolean chanceToFindRelic(AAttributes attacker) {
        ...
        if (ranNum < num) {
            ...
            switch (generateRelics()) {
                case 0 -> {
                    attacker.setStrength(attacker.getStrength() +
                            relicOfFury.getRelicStrength());
                    relicOfFury.setId(7);
                    db.updatePlayer(attacker);
                    db.insertRelic(relicOfFury, attacker);
                }
                case 1 -> {
                    attacker.setSpirit(attacker.getSpirit() +
                            relicOfLife.getRelicSpirit());
                    relicOfLife.setId(8);
                    db.updatePlayer(attacker);
                    db.insertRelic(relicOfLife, attacker);
                }
                case 2 -> {
                    attacker.setHealth(attacker.getHealth() +
                            relicOfProtection.getRelicHealth());
                    relicOfProtection.setId(9);
                    db.updatePlayer(attacker);
                    db.insertRelic(relicOfProtection, attacker);
                }
            }
            relicFound = true;
        } else {
